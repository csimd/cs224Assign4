...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, (prev_h, prev_c) = self.encoder(X)
        X = pad_packed_sequence(enc_hiddens)[0]
        X = X.permute(1, 0, 2)
        init_decoder_hidden = self.h_projection(torch.cat((prev_h[0], prev_h[1]), 1))
        init_decoder_cell   = self.c_projection(torch.cat((prev_c[0], prev_c[1]),  1))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        O_t = self.dropout(torch.tanh(self.combined_output_projection(\
            torch.cat((dec_hidden, a_t), 1))))
